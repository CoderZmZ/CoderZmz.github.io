...
extern "C" void slave_csr_spmv(cs_sp *cs);
extern "C" void slave_csr_pre_spmv(cs_sp* cs);
...
void csr_spmv(const CsrMatrix &csr_matrix, double *vec, double *result) {
    ...
    CRTS_init();
	cs_sp c_s;
    c_s.rows    =    csr_matrix.rows;
    c_s.row_off = csr_matrix.row_off;
    c_s.vec     =                vec;
    c_s.cols    =    csr_matrix.cols;
    c_s.result  =             result;
    c_s.data    =    csr_matrix.data;

	athread_spawn(slave_csr_spmv, &c_s);
	athread_join();
}
...
void csr_precondition_spmv(const CsrMatrix &csr_matrix, double *vec, double *val, double *result) {
    ...
    CRTS_init();
	cs_sp c_p_s;
    c_p_s.rows    =    csr_matrix.rows;
    c_p_s.row_off = csr_matrix.row_off;
    c_p_s.vec     =                vec;
    c_p_s.cols    =    csr_matrix.cols;
    c_p_s.result  =             result;
    c_p_s.val    =                 val;

	athread_spawn(slave_csr_pre_spmv, &c_p_s);
	athread_join();
}
